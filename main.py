...
class TrainingPlannerApp:
    ...
    def create_widgets(self):
        ...
        self.type_entry.grid(row=1, column=1, sticky="w", pady=2)

        ttk.Label(input_frame, text="Длительность (мин):").grid(row=2, column=0, sticky="w", pady=2)
        self.duration_entry = ttk.Entry(input_frame, width=25)
        ...
        filter_frame.grid(row=1, column=0, columnspan=2, padx=10, pady=(0, 10), sticky="ew")
        ...
        self.filter_type_entry.grid(row=0, column=1, sticky="ew", padx=(0, 5))

        ttk.Label(filter_frame, text="По дате:").grid(row=1, column=0, sticky="w")
        self.filter_date_entry = ttk.Entry(filter_frame, textvariable=self.filter_date_var)
        self.filter_date_entry.grid(row=1, column=1, sticky="ew", padx=(0, 5))

        ttk.Button(filter_frame, text="Применить фильтр", command=self.apply_filter).grid(
            row=2, column=0, columnspan=2, pady=(5, 2), sticky="ew")
        
        ttk.Button(filter_frame, text="Сбросить фильтр", command=self.reset_filter).grid(
            row=3, column=0, columnspan=2, pady=(2, 5), sticky="ew")
        ...
        self.tree.column("duration", anchor="center", width=160)
        ...
        json_frame.grid(row=5, column=0, columnspan=2)

        ttk.Button(json_frame, text="Сохранить в JSON", command=self.save_data).grid(
            row=0, column=0, padx=(10, 5), pady=(0, 10))
        
        ttk.Button(json_frame, text="Загрузить из JSON", command=self.load_data).grid(
    ...
    def add_training(self):
        ...
        self.tree.insert("", "end", values=(date_str.strip(), type_str.strip(), duration_str.strip()))
    ...
    def validate_inputs(self, date_str: str, duration_str: str):
        try:
            # Проверка даты (строгий формат ГГГГ-ММ-ДД)
